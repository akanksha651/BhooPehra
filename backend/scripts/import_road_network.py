...
def validate_database(connection) -> None:
    ...
    print(
        f"Created timestamps   : {summary['created_at_count']:,}"
    )
    print(
        f"Updated timestamps   : {summary['updated_at_count']:,}"
    )
    print(
    ...
    if summary["total"] != summary["created_at_count"]:
        raise RuntimeError(
            "Database validation failed: some rows have no created_at."
        )

    if summary["total"] != summary["updated_at_count"]:
        raise RuntimeError(
            "Database validation failed: some rows have no updated_at."
        )
